...
class MultiWOZParser:
    ...
    def __init__(self, directory="", version="2.0", simplification_level=2, include_action_specs=True,
                 keep_book_booking=False, keep_reqmore=False, keep_nooffer=False, aggregate_actions=True,
                 add_status_slots=True, debug=False):
        """
        ...
        self.keep_book_booking = keep_book_booking
        self.keep_reqmore = keep_reqmore
        self.keep_nooffer = keep_nooffer
        self.aggregate_actions = aggregate_actions
        self.include_action_specs = include_action_specs
        self.slot_parser = SlotParser(simplify=simplification_level)

        if debug:
            num_debug_stories = 10

            # We want to use only the first 10 examples, so we store
            # them under ".../data.debug.json"
            debug_story_names = []
            debug_data_file_name = os.path.abspath(
                os.path.join(unpacked_dir, "data.debug.json")
            )
            debug_acts_file_name = os.path.abspath(
                os.path.join(unpacked_dir, "dialogue_acts.debug.json")
            )
            if not os.path.exists(debug_data_file_name):
                # Copy the first 10 examples from the data file
                print("DEBUG: Generating debug data file...")
                with open(self.data_file_name, "r") as read_file:
                    with open(debug_data_file_name, "w") as write_file:
                        count = 0
                        while True:
                            line = read_file.readline()
                            if line.rstrip().endswith(".json\": {"):
                                # This line starts a new story
                                count += 1
                                if count <= num_debug_stories:
                                    debug_story_names += [line.strip("\": {\n")]
                            if count <= num_debug_stories:
                                if count == num_debug_stories and line.startswith("    },"):
                                    # Avoid writing the last comma
                                    write_file.write("    }")
                                else:
                ...
                # Find the entries in the acts file that correspond to the first
                # 10 data examples (files are in different order, so we have to
                # load all acts
                print("DEBUG: Generating debug acts file...")
                with open(self.acts_file_name, "r") as read_file:
                    with open(debug_acts_file_name, "w") as write_file:
                        count = 0
                        recording = True
                        while True:
                            line = read_file.readline()
                            if line.startswith("    \""):
                                # This line starts a new story
                                story_name = line.strip("\": {\n") + ".json"
                                if story_name in debug_story_names:
                                    count += 1
                                    recording = True
                                else:
                                    recording = False
            ...
            self.data_file_name = debug_data_file_name
            self.acts_file_name = debug_acts_file_name
    ...
    def parse_action(self, action_list):
        if self.aggregate_actions:
            ...
            actions = self._parse_action_individual(action_list)
        ...
        for action in actions:
            ...
            # Substitute book_booking_DOMAIN with advise_DOMAIN, since we have `action_booking` for that, and
            # book_booking sometimes indicates that the wizard tells the user about reference numbers, etc.
            if not self.keep_book_booking and action.name == "book" and action.tags and "booking" in action.tags:
                action.name = "inform"
                action.tags = None

            # Substitute nooffer_DOMAIN with nobook_booking_DOMAIN
            if not self.keep_nooffer and action.name == "nooffer" and action.tags is None:
                self.name = "nobook"
                self.tags = ["booking"]

            # Remove repetitions of actions and
            # ignore `reqmore_general` if it is not the only action
            if (actions2 == [] or action.to_string() != actions2[-1].to_string()) and \
                    (action.name != "reqmore" or len(actions) == 1 or self.keep_reqmore):
    ...
    def _parse_action_aggregated(self, action_list):
        ...
        for head, specs in action_groups.items():
            ...
            string = "   - " + activity
            if self.include_action_specs:
                # Sort specs (name, price, etc.) alphabetically
    ...
    def _parse_action_individual(self, action_list):
        actions = []
        if type(action_list) is not dict:
            if type(action_list) is str and action_list == "No Annotation":
                log_problem({"type": "no_annotation"})
        ...
        for base_action, slots in action_list.items():
            ...
            if "-" in base_action:
                ...
                # Single-letter actions like "A" or "N" are actually never requested
                log_problem({"type": "one_letter_action", "base_action": base_action})
                return []
            ...
            for slot in slots:
                ...
                full_action_name = action_name + "_" + spec + domain.lower()

                action = DialogTurn.from_string("- " + full_action_name)
                action.store_in_domain_info()
    ...
    def _substitute_entity(self, string):
        """
        Replaces times, numbers, and weekdays in the given string with special tokens
    ...
    def find_example_action_text(self, action_string, single=False, max_examples=-1, include_user=False):
        examples = []

        for name in self.story_names:
            dialog = self.data[name]
            log = dialog["log"]

            count_wiz = 0
            if include_user:
                last_user_utterance = "EMPTY"
                ut = IntentParser(self.slot_parser, add_status_slots=self.add_status_slots)
            for step in log:
                if len(step["metadata"]) == 0:  # User-texts don't have metadata
                    if include_user:
                        last_user_utterance = step["text"]
                else:
                    # Infer user utterance from new information
                    count_wiz += 1

                    if include_user:
                        # noinspection PyUnboundLocalVariable
                        turns_from_user = ut(step["metadata"])

                        # If this is the end of the dialog, then we assume the user's `inform{}` (no slots) is actually
                        # a `bye`
                        if count_wiz * 2 == len(log) and len(turns_from_user) == 1 and not turns_from_user[0].slots:
                            turns_from_user[0].name = "bye"

                        intent = turns_from_user[2:]

                        if intent == action_string:
                            # noinspection PyUnboundLocalVariable
                            examples = examples + [last_user_utterance]

                    # Wizard's information
                    if str(count_wiz) in self.acts[name[:-5]]:
                        action = self.acts[name[:-5]][str(count_wiz)]
                        num_problems = len(multiwoz.domain_info.problems)
                        actions = self.parse_action(action)

                        if len(multiwoz.domain_info.problems) > num_problems:
                            break

                        if single and len(actions) > 1:
                            break

                        for a in actions:
                            if a.is_action:
                                if a.full_name == action_string:
                                    examples = examples + [step["text"]]
    ...
    def demo_raw_actions(self, name):
        dialog = self.data[name]
        log = dialog["log"]

        print(colored(f"## story_{name[:-5]}", "green"))

        count_wiz = 0
        for step in log:
            if len(step["metadata"]) == 0:  # User-texts don't have metadata
                # User's text
                print(colored("U:  " + step["text"], "blue"))
            else:
                # Infer user utterance from new information
                count_wiz += 1

                # Wizard's text
                print(colored("W:  " + step["text"], "green"))

                # Wizard's information
                if str(count_wiz) in self.acts[name[:-5]]:
                    actions = sorted(list(self.acts[name[:-5]][str(count_wiz)]))
                    for action in actions:
                        print(colored(f"  {action}", "magenta"))
                else:
                    print(colored("<NO ANNOTATION>", "red"))


if __name__ == '__main__':
    parser = MultiWOZParser(simplification_level=0, add_status_slots=True)
    print(parser.parse_story_e2e("MUL0129.json", verbose=0))
    print(parser.parse_story_e2e("MUL2168.json", verbose=0))
    exit(0)
